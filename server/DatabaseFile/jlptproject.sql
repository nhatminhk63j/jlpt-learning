-- phpMyAdmin SQL Dump
-- version 5.0.2
-- https://www.phpmyadmin.net/
--
-- Máy chủ: 127.0.0.1
-- Thời gian đã tạo: Th4 20, 2020 lúc 12:46 PM
-- Phiên bản máy phục vụ: 10.4.11-MariaDB
-- Phiên bản PHP: 7.4.4

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Cơ sở dữ liệu: `jlptproject`
--

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `exam`
--

CREATE TABLE `exam` (
  `CodeExam` varchar(255) NOT NULL,
  `Level` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

--
-- Đang đổ dữ liệu cho bảng `exam`
--

INSERT INTO `exam` (`CodeExam`, `Level`) VALUES
('12234234', 'N3'),
('11556', 'N4');

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `grammar`
--

CREATE TABLE `grammar` (
  `id` int(11) NOT NULL,
  `CodeExam` varchar(255) NOT NULL,
  `content` text NOT NULL,
  `Level` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

--
-- Đang đổ dữ liệu cho bảng `grammar`
--

INSERT INTO `grammar` (`id`, `CodeExam`, `content`, `Level`) VALUES
(44, '12234234', '1236548', 'N3'),
(45, '11556', 'aaaaaaaaâ', 'N4');

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `grammarpractice`
--

CREATE TABLE `grammarpractice` (
  `id` int(11) NOT NULL,
  `level` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `kanjipractice`
--

CREATE TABLE `kanjipractice` (
  `id` int(11) NOT NULL,
  `level` varchar(255) NOT NULL,
  `name` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `level`
--

CREATE TABLE `level` (
  `name` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

--
-- Đang đổ dữ liệu cho bảng `level`
--

INSERT INTO `level` (`name`) VALUES
('N1'),
('N2'),
('N3'),
('N4'),
('N5');

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `listenning`
--

CREATE TABLE `listenning` (
  `id` int(11) NOT NULL,
  `CodeExam` varchar(255) NOT NULL,
  `filename` text NOT NULL,
  `content` text NOT NULL,
  `title` varchar(255) NOT NULL,
  `Level` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `questiongrammar`
--

CREATE TABLE `questiongrammar` (
  `id` int(11) NOT NULL,
  `title` text NOT NULL,
  `answerA` varchar(255) NOT NULL,
  `answerB` varchar(255) NOT NULL,
  `answerC` varchar(255) NOT NULL,
  `answerD` varchar(255) NOT NULL,
  `answerTrue` varchar(255) NOT NULL,
  `idGrammar` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

--
-- Đang đổ dữ liệu cho bảng `questiongrammar`
--

INSERT INTO `questiongrammar` (`id`, `title`, `answerA`, `answerB`, `answerC`, `answerD`, `answerTrue`, `idGrammar`) VALUES
(11, 'Hello Nhật', 'Nhật có đang xem cái này không', 'Hay lại là thằng Hào lz', 'Hào êy Lên Hà Lội chưa', 'Nhà đang bẩn lắm lên dọn đi', 'C', 44),
(12, 'Lên dọn nhà nào', 'Huy ăn cứt', 'Cứt ăn Huy', 'Đây là đáp án của thằng Huy', 'Sao thằng Hào lại nhẩy vào đọc', 'A', 44),
(13, 'Có phải mình đang ở đây không', 'Làm gì để có người yêu đây', 'Nhật có người yêu cả rồi', 'Sao mình vẫn chưa có người yêu', 'Mình không thể như HUy Đc', 'A', 44),
(14, 'Bạn ơi,', 'Bạn đnag làm gì thế', 'Có đang đi ỉa không', 'oôtke', 'Hêlo', 'A', 44);

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `questionlistening`
--

CREATE TABLE `questionlistening` (
  `id` int(11) NOT NULL,
  `titile` text NOT NULL,
  `answerA` varchar(255) NOT NULL,
  `answerB` varchar(255) NOT NULL,
  `answerC` varchar(255) NOT NULL,
  `answerD` varchar(255) NOT NULL,
  `answerTrue` varchar(255) NOT NULL,
  `idListening` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `questionpractice`
--

CREATE TABLE `questionpractice` (
  `id` int(11) NOT NULL,
  `question` text NOT NULL,
  `answer1` varchar(255) NOT NULL,
  `answer2` varchar(255) NOT NULL,
  `answer3` varchar(255) NOT NULL,
  `answer4` varchar(255) NOT NULL,
  `result` int(11) NOT NULL,
  `type` varchar(255) NOT NULL,
  `level` varchar(255) NOT NULL,
  `idRLG` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

--
-- Đang đổ dữ liệu cho bảng `questionpractice`
--

INSERT INTO `questionpractice` (`id`, `question`, `answer1`, `answer2`, `answer3`, `answer4`, `result`, `type`, `level`, `idRLG`) VALUES
(19, 'A', 'A', 'A', 'A', 'A', 1, 'vocabulary', 'N5', 17),
(20, 'A', 'A', 'A', 'A', 'A', 1, 'vocabulary', 'N5', 17),
(21, 'A', 'A', 'A', 'A', 'A', 1, 'vocabulary', 'N4', 18),
(22, 'A', 'A', 'A', 'A', 'A', 1, 'vocabulary', 'N4', 18);

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `questionreading`
--

CREATE TABLE `questionreading` (
  `id` int(11) NOT NULL,
  `title` varchar(255) NOT NULL,
  `answerA` varchar(255) NOT NULL,
  `answerB` varchar(255) NOT NULL,
  `answerC` varchar(255) NOT NULL,
  `answerD` varchar(255) NOT NULL,
  `answerTrue` varchar(255) NOT NULL,
  `idReading` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `reading`
--

CREATE TABLE `reading` (
  `id` int(11) NOT NULL,
  `CodeExam` varchar(255) NOT NULL,
  `title` varchar(255) NOT NULL,
  `content` text NOT NULL,
  `Level` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `typeexam`
--

CREATE TABLE `typeexam` (
  `id` int(11) NOT NULL,
  `name` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

--
-- Đang đổ dữ liệu cho bảng `typeexam`
--

INSERT INTO `typeexam` (`id`, `name`) VALUES
(1, 'Reading'),
(2, 'Grammar'),
(3, 'Listenning');

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `user`
--

CREATE TABLE `user` (
  `id` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- --------------------------------------------------------

--
-- Cấu trúc bảng cho bảng `vocabularypractice`
--

CREATE TABLE `vocabularypractice` (
  `id` int(11) NOT NULL,
  `level` varchar(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

--
-- Đang đổ dữ liệu cho bảng `vocabularypractice`
--

INSERT INTO `vocabularypractice` (`id`, `level`) VALUES
(17, 'N5'),
(18, 'N4');

--
-- Chỉ mục cho các bảng đã đổ
--

--
-- Chỉ mục cho bảng `exam`
--
ALTER TABLE `exam`
  ADD PRIMARY KEY (`CodeExam`),
  ADD KEY `Level` (`Level`) USING BTREE;

--
-- Chỉ mục cho bảng `grammar`
--
ALTER TABLE `grammar`
  ADD PRIMARY KEY (`id`),
  ADD KEY `levelG` (`Level`),
  ADD KEY `CodeExam` (`CodeExam`) USING BTREE;

--
-- Chỉ mục cho bảng `grammarpractice`
--
ALTER TABLE `grammarpractice`
  ADD PRIMARY KEY (`id`);

--
-- Chỉ mục cho bảng `kanjipractice`
--
ALTER TABLE `kanjipractice`
  ADD PRIMARY KEY (`id`);

--
-- Chỉ mục cho bảng `level`
--
ALTER TABLE `level`
  ADD PRIMARY KEY (`name`);

--
-- Chỉ mục cho bảng `listenning`
--
ALTER TABLE `listenning`
  ADD PRIMARY KEY (`id`),
  ADD KEY `LevelL` (`Level`) USING BTREE,
  ADD KEY `codex` (`CodeExam`);

--
-- Chỉ mục cho bảng `questiongrammar`
--
ALTER TABLE `questiongrammar`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idG` (`idGrammar`);

--
-- Chỉ mục cho bảng `questionlistening`
--
ALTER TABLE `questionlistening`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idlisten` (`idListening`);

--
-- Chỉ mục cho bảng `questionpractice`
--
ALTER TABLE `questionpractice`
  ADD PRIMARY KEY (`id`);

--
-- Chỉ mục cho bảng `questionreading`
--
ALTER TABLE `questionreading`
  ADD PRIMARY KEY (`id`),
  ADD KEY `igRead` (`idReading`);

--
-- Chỉ mục cho bảng `reading`
--
ALTER TABLE `reading`
  ADD PRIMARY KEY (`id`),
  ADD KEY `levelR` (`Level`),
  ADD KEY `codeR` (`CodeExam`);

--
-- Chỉ mục cho bảng `typeexam`
--
ALTER TABLE `typeexam`
  ADD PRIMARY KEY (`id`);

--
-- Chỉ mục cho bảng `user`
--
ALTER TABLE `user`
  ADD PRIMARY KEY (`id`);

--
-- Chỉ mục cho bảng `vocabularypractice`
--
ALTER TABLE `vocabularypractice`
  ADD PRIMARY KEY (`id`);

--
-- AUTO_INCREMENT cho các bảng đã đổ
--

--
-- AUTO_INCREMENT cho bảng `grammar`
--
ALTER TABLE `grammar`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=46;

--
-- AUTO_INCREMENT cho bảng `grammarpractice`
--
ALTER TABLE `grammarpractice`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT cho bảng `kanjipractice`
--
ALTER TABLE `kanjipractice`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT cho bảng `listenning`
--
ALTER TABLE `listenning`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;

--
-- AUTO_INCREMENT cho bảng `questiongrammar`
--
ALTER TABLE `questiongrammar`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=16;

--
-- AUTO_INCREMENT cho bảng `questionlistening`
--
ALTER TABLE `questionlistening`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT cho bảng `questionpractice`
--
ALTER TABLE `questionpractice`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=23;

--
-- AUTO_INCREMENT cho bảng `questionreading`
--
ALTER TABLE `questionreading`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;

--
-- AUTO_INCREMENT cho bảng `reading`
--
ALTER TABLE `reading`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=12;

--
-- AUTO_INCREMENT cho bảng `typeexam`
--
ALTER TABLE `typeexam`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;

--
-- AUTO_INCREMENT cho bảng `user`
--
ALTER TABLE `user`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT cho bảng `vocabularypractice`
--
ALTER TABLE `vocabularypractice`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=19;

--
-- Các ràng buộc cho các bảng đã đổ
--

--
-- Các ràng buộc cho bảng `exam`
--
ALTER TABLE `exam`
  ADD CONSTRAINT `Level` FOREIGN KEY (`Level`) REFERENCES `level` (`name`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Các ràng buộc cho bảng `grammar`
--
ALTER TABLE `grammar`
  ADD CONSTRAINT `CodeExam` FOREIGN KEY (`CodeExam`) REFERENCES `exam` (`CodeExam`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `levelG` FOREIGN KEY (`Level`) REFERENCES `exam` (`Level`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Các ràng buộc cho bảng `listenning`
--
ALTER TABLE `listenning`
  ADD CONSTRAINT `codex` FOREIGN KEY (`CodeExam`) REFERENCES `exam` (`CodeExam`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `idlevel` FOREIGN KEY (`Level`) REFERENCES `exam` (`Level`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Các ràng buộc cho bảng `questiongrammar`
--
ALTER TABLE `questiongrammar`
  ADD CONSTRAINT `idG` FOREIGN KEY (`idGrammar`) REFERENCES `grammar` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Các ràng buộc cho bảng `questionlistening`
--
ALTER TABLE `questionlistening`
  ADD CONSTRAINT `idlisten` FOREIGN KEY (`idListening`) REFERENCES `listenning` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Các ràng buộc cho bảng `questionreading`
--
ALTER TABLE `questionreading`
  ADD CONSTRAINT `igRead` FOREIGN KEY (`idReading`) REFERENCES `reading` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Các ràng buộc cho bảng `reading`
--
ALTER TABLE `reading`
  ADD CONSTRAINT `codeR` FOREIGN KEY (`CodeExam`) REFERENCES `exam` (`CodeExam`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `levelR` FOREIGN KEY (`Level`) REFERENCES `exam` (`Level`);
COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
